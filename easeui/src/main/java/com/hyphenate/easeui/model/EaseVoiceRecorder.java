...
public class EaseVoiceRecorder {
    ...
    static final String EXTENSION = ".m4a";
    ...
    public String startRecording(Context appContext) {
        ...
        try {
            ...
            // User.getVoicePath());
            if (flag){
                File file = new File(Environment.getExternalStorageDirectory(), "/oking/mission_voice");
                if (!file.exists()){
                    file.mkdirs();
                }
                voiceFileName = UUID.randomUUID().toString()+EXTENSION;
                voiceFilePath = file.getPath()+"/"+voiceFileName;
            }else {
    ...
    public void setRecordTag(boolean flag){
        this.flag = flag;
    }
}
