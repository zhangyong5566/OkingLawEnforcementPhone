        ...
        gdWaterService.login(acount, pwd)
                ...
                .concatMap(new Function<ResponseBody, Observable<ResponseBody>>() {
                    ...
                    public Observable<ResponseBody> apply(ResponseBody responseBody) throws Exception {
                        ...
                        GreenUser uniqueOrThrow = GreenDAOMannager.getInstence().getDaoSession().getGreenUserDao().queryBuilder().where(GreenUserDao.Properties.Userid.eq(userid)).unique();
                        sp.edit().putString(acount,userid).commit();
                        if (uniqueOrThrow!=null) {
                            mUid = uniqueOrThrow.getId();
                            mGreenUser.setId(mUid);

                            GreenDAOMannager.getInstence().getDaoSession().getGreenUserDao().update(mGreenUser);
                            Log.i("Oking", "登录成功更新数据");
                        } else {
                            mUid = GreenDAOMannager.getInstence().getDaoSession().getGreenUserDao().insert(mGreenUser);
                ...
                .subscribe(new Consumer<ResponseBody>() {
                    ...
                    public void accept(Throwable throwable) throws Exception {
                        Log.i("Oking","登录失败"+throwable.getMessage());
                        //尝试离线登录
 ...
 */
    private void loginOfLine() {
        Schedulers.io().createWorker().schedule(new Runnable() {
            @Override
            public void run() {
                GreenUser unique = GreenDAOMannager.getInstence().getDaoSession().getGreenUserDao().queryBuilder()
